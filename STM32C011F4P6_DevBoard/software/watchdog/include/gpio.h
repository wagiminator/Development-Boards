// ===================================================================================
// Basic GPIO Functions for STM32C011 and STM32C031                           * v1.0 *
// ===================================================================================
//
...
//   PA0, PA1, PA2, PA3, PA4, PA5, PA6, PA7, PA8, PA11, PA12, PA13, PA14, 
//   PB0, PB1, PB2, PB10, PB11, PB12.
//
...
#define ADC_VREFCAL         (*(__I uint16_t*)(0x1FFF756A))    // at 3000mV
#define ADC_TSCAL1          (*(__I uint16_t*)(0x1FFF7568))    // at 30°C
#define ADC_TSCAL2          (*(__I uint16_t*)(0x1FFF758A))    // at 130°C
...
#define ADC_SLOPE           10363 // temperature slope: 2.53mV/°C * 4096
...
#define ADC_input(PIN) {                \
  ...
  ((PIN>=PA0 )&&(PIN<=PA8 ) ? ( ADC1->CHSELR = (uint32_t)1<<((PIN)&15)     ) : \
  ((PIN>=PA11)&&(PIN<=PA14) ? ( ADC1->CHSELR = (uint32_t)1<<((PIN)&15)     ) : \
  ((PIN>=PB0 )&&(PIN<=PB2 ) ? ( ADC1->CHSELR = (uint32_t)1<<(((PIN)&7)+17) ) : \
  ((PIN>=PB10)&&(PIN<=PB12) ? ( ADC1->CHSELR = (uint32_t)1<<(((PIN)&7)+18) ) : \
  (0)))));                            \
...
static inline void ADC_input_TEMP(void) {
  ...
  ADC1->CHSELR = (uint16_t)1<<9;                // set ADC channel 9
...
static inline void ADC_input_VREF(void) {
  ADC1->ISR = ADC_ISR_CCRDY;                    // clear config ready flag
  ADC1->CHSELR = (uint16_t)1<<10;               // set ADC channel 10
  while(!(ADC1->ISR & ADC_ISR_CCRDY));          // wait until configured
}
...
static inline int8_t ADC_read_TEMP(void) {
  ...
  return((tdata-ADC_TSCAL1)*ADC_CVOLT/(ADC_SLOPE)+ADC_CTEMP1);
}
