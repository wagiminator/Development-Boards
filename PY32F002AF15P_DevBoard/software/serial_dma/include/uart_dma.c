...
#define UART_RX_hptr (UART_RX_BUF_SIZE - DMA_CHAN->CNDTR)
...
// Set GPIO pins
  ...
  SYSCFG->CFGR3  |= 0b00110 << DMA_POS;             // set USART1 RX as trigger
  DMA_CHAN->CNDTR = (uint16_t)UART_RX_BUF_SIZE;     // ring buffer size
  DMA_CHAN->CMAR  = (uint32_t)UART_RX_buffer;       // memory address
  DMA_CHAN->CPAR  = (uint32_t)&USART1->DR;          // peripheral address
  DMA_CHAN->CCR   = DMA_CCR_MINC                    // increment memory address
